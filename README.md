...
一个给 AI Agent 用的中文写作 lint Skill：把「AI 味」的中文洗回「像人说的中文」。

它不生成新内容，只清洗已有的中文——保事实、去 AI 味、加中文味、不许造词充深刻。把稿子丢给它，它按一套清单走一遍，告诉你动了哪一刀、命中了哪几招、哪几处建议你自己决定删不删。
...
```
haohao-shuohua/
├── SKILL.md                            # 技能主文件:四条压顶原则、三档场景、五段主流程
└── references/
    ├── symptom-dictionary.md           # 症状词典:AI 味四层病灶 + 减法五刀
    ├── chinese-four-principles.md      # 中文四原则:动词发力/具体先于抽象/短句串珠/留白
...
整篇文章都要围着一个词转，先把它说清楚，不然后面越讲越飘。

**欧化中文，就是把英文的句法、词序、结构硬套进中文里，让中文外表还是中文，骨子却长成英文的样子。**「翻译腔」是它的口语说法。

这个词最早出现在语言学家[王力](https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%8A%9B_(1922%E5%B9%B4)) 1944 年写的《中国现代语法》里，他专门辟了一章叫「欧化的语法」，是汉语学界对这件事最早的系统诊断：白话文运动之后，一批从英文和日文翻译过来的语法结构直接进了汉语，王力把它们一条一条列出来，比如滥用「们」字复数、被动句泛滥、判断句一律加「是」、名词后面拖一串「的」字定语。是不是很熟悉？他讲的这些，正是 AI 味的一个重要来源。
...
他讲，中文本来靠动词发力，一句话动词一响，事情就成立；英文靠名词和 be 动词撑架子。欧化就是把英文那副架子搬到中文里，动词躲进名词后面，句子越来越绵软。他自己举过一个到现在都常被引用的例子：中文本来会说「他因家境欠佳，不能上大学」，欧化后非要写成「由于他的家境不太好，使得他不能读大学」，多出来的字全在给动词让路。
...
## 那为什么 AI 一出来这个老病就浓到受不了

同样的病一百年，AI 一出来就浓到人一眼就烦。三个原因，都很硬。

**训练数据里中文本来就少。** OpenAI 公开的 GPT-3 训练数据统计里，英语占 92.65%，简体中文 0.09905%，繁体中文 0.01975%，中文合起来大约 0.12%（[链接](https://github.com/openai/gpt-3/blob/master/dataset_statistics/languages_by_word_count.csv)）。英文是中文的七百多倍。连千分之一都不到，为什么 GPT-3 还能说中文？两层原因：一是 GPT-2 起换成了字节级 BPE 分词，任何 Unicode 字符都能被拆成字节表示，中文不需要单独词表就能进模型，代价是同一句中文往往吃掉英文两到三倍的 tokens；二是跨语言迁移，模型在英文上学到的语义和推理能力，会顺带迁到中文这类它见得少的语言上。技术上进得来，不代表说得地道。最近 GPT-Live 说中文，不少人一听就皱眉：ABC 味太冲了。

**它能读到的「好中文」本身就是欧化最重的那一批。** 论文、教科书、企业内部文档、公众号里那种一本正经的「深度好文」，大多是过去几十年翻译体喂出来的写法。模型学的是这个，写出来的也是这个。汪曾祺、阿城那种中文在训练集里的密度接近于零，你不能指望它写得出来。

**后期训练又教它专门去讨好评分员。** RLHF 阶段找标注员给回答打分，什么样的回答分高？结构完整、四平八稳、句式工整、用词「专业」。标注员看重什么，模型就长什么样：靠句式撑门面、靠名词堆气势、靠对仗句显得深刻。真人工程师写的直给短句，在标注员眼里反而「不够完整」、「不够正式」。事情说没说清楚不那么重要，分先拿到手再说。**这一层严格说不是语言的病，是训练机制的病，但结果长在语言上。**

三样叠起来：中文语料本来就少，里面高质量的那部分欧化最重，训练目标又专门奖励讨好。所以核心结论其实很不给面子：**AI 味不是 AI 自己带来的，是我们教它的**。评分员每按一次「这条更专业」，就在给欧化中文投一票；模型只是欧化中文的「集大成者」。

鲁迅一百年前追问过一句「从来如此，便对么？」这句话今天该转过来再问一遍：奖励堆出来的那些句子，到底还是不是中文？答案不在评分表里，藏在句子本身里。

## 拆开看：欧化中文的几种典型病灶

下面几种病灶，是 AI 中文里最常见的几副面孔。每一副都给一组对照：**英文原句 → 直译回中文（也就是 AI 常写法）→ 中文本来会怎么说**。三行摆开，病根一眼就见。

**病灶一 · 动词名词化**

- 英文原句：*We conducted a thorough discussion of the proposal.*
- 直译回中文（=AI 常写法）：我们对该方案进行了充分的讨论。
- 中文本来的说法：我们把这个方案讨论透了。
...
英文靠从句、破折号、插入语撑住一个长句；中文没有关系代词，硬撑就得靠一串「的」和「，」串下去，读起来像喘不上气。破折号是从英文的 em dash 直接搬过来的用法，原生中文用得极省。顺便自首一句：本文自己也没做到干净，前几版里破折号也偷偷用了不少，这一版专门盘了一遍。
...
标注员喜欢什么，模型就长什么。这一类不是翻译腔，是「表演给评分员看」，句子骨架未必是英文，姿态却是刷分的。

互联网黑话（「抓手」「颗粒度」「底层逻辑」「心智」）是同一手法的方言版：用一个不精确但显得专业的词，替换一个精确但显得普通的词。「抓手」替「办法」，「颗粒度」替「多细」，「底层逻辑」替「根本原因」。词换上去，事情反而说糊了，发言人却显得更资深。AI 学会了这套，因为语料里密度高，评分员也觉得听着「专业」。

## 把中文本来的写法加回来

光减 AI 味不够，减完稿子会瘦成空壳，像去了油的白水汤，还得把中文本来的写法补回来。四条，每条都拿一句名家名句和 AI 味会写成什么样成对摆出来看。这样比较起来省事：中文那一边不用担心谁质疑，就是原文；剩下要吐槽的只有 AI 味那一边。
...
为什么句号一按事就完了，会比一路拖下去更像中文？**因为中文是靠节奏走的。**中文一个字一个音节，字字都要落地；短句一节一节推进，节奏是断的，气反而是连的。英文不一样，它有语调起伏这条轨道，一路 and, which, that 拉一长串下去，读者靠 intonation 就能跟住；中文没这条轨道，一句拖长，气就散了。

这不是我们的新发现，古人早就讲透了。韩愈《答李翊书》一句「气盛则言之短长，声之高下，皆宜」；桐城派学古文特重朗诵，就是拿耳朵校节奏。朱光潜在《谈文学·散文的声音节奏》里也把话说死了：「段落的起伏开合，句的长短，字的平仄，文的骈散，都与声音有关。」汪曾祺讲得最白：「语言的奥秘，不过是长句与短句的搭配。」
...
汪曾祺讲完《受戒》那么长一个故事，结尾就一句话，一个日期加一个「梦」字，全篇的分量就压在这一句上。他不解释这是什么梦、为什么写、有什么意义，因为他信任读者能自己接。AI 味相反：讲完一件事，一定要补一句「我刚才讲了这件事，而且很重要」，怕读者跟不上。段末的「综上所述」「也就是说」「换言之」「由此可见」「这标志着」几乎都可以直接删。**留白的前提是尊重读者；AI 没有这个前提，它要的是评分员看到「结构完整」。**

## 把上面这些做成一份可以上手的手术清单

前面把病理讲清了，下面把它变成能上手改稿的手术清单。分三步：先自检、再下刀减、最后补上中文原本的写法。全篇的例子从上面几段真实 AI 中文里抓，以及一篇之前 AI 写的关于「验证驱动开发」的稿子。改的时候尽量给出改前改后，让手感落地。

## 只用一个自检法就够：回译测试

诊断 AI 味最简单的方法只有一条，叫回译测试：
...
一句中文越容易顺畅地翻回英文，它就越像英文；越难翻，就越像中文。这个测试不需要工具、不需要打分，写完一段自己念一遍，心里回译一下就知道。

下面所有条目，都是这一条测试的具体化。

## 症状清单：先做诊断

下笔改之前，先扫一遍稿子，把这些症状标出来。看到就画圈，不用一次全改，先看看密度。密度高就说明这段整体不对，不是改几个词能救的。
...
3. **标红的段先做减法。**五刀：破折号、名词化、后缀病、前者后者、长复合句。机械做完再看。
4. **减完做加法四问。**动词扛住了没？有具体没？句子拆开没？段末多话删了没？
5. **最后一遍念出声。**中文的病，眼睛看不出，耳朵能听出。念到哪里卡壳、拗口、气喘不上，就是那里还没治干净。

## 把这套方法沉成了一个可复用的 Skill

上面这一整套（回译测试 + 症状清单 + 减法五刀 + 加法四问 + 短流程），我把它沉成了一个可以直接调用的 Skill，叫「好好说话」（haohao-shuohua）。写稿的时候不用再自己一条条对，把稿子丢给它，它按上面的清单走一遍，改回来告诉你动了哪一刀、命中了哪几招、哪几处建议你自己决定删不删。

Skill 里比正文多几层兜底：先锁事实账本（数字、术语、谓词方向不许动）再动刀；力度和范围两轴独立配置（轻档改对话、中档改周报、重档改硬文）；改完自己两遍回读（一遍对事实、一遍查残留 AI 味）；批评类文章自证一致（自己批过的每一条病都 grep 一遍自己的稿子）。

安装和使用见本页顶部「[安装](#安装)」一节。

## 写在最后

AI 味不是 AI 的错，是一百年欧化中文欠账加一套讨好机制训出来的合力。它不会自动消失，反而正在变成默认：我们习惯都用 AI 写文章，写的这些文章又被 AI 拿去训练，几个版本之后，AI 认识到的中文可能就是欧化中文这个样子了。
